...
void example_plot(TString process, TString unctype, TString histName, TString leg_input, int bin, bool axisrange=false, double xmin=1, double xmax=100 ){
  ...
  TText *text = header->AddText("CMS        #sqrt{s} = 13 TeV, 35.9 fb^{-1}");
  ...
  TMean1.DrawTextNDC(.77,.750,mean1);
  ...
  TMean2.DrawTextNDC(.77,.650,mean2);
  ...
  TMean3.DrawTextNDC(.77,.550,mean3);
  ...
  hRatio_Up->Add(h2);
  hRatio_Up->Divide(h1); 
  hRatio_Up->SetMarkerStyle(20); 
  hRatio_Up->SetMarkerSize(0.8);
  ...
  hRatio_Up->GetYaxis()->SetTitleSize(0.15); 
  hRatio_Up->GetXaxis()->SetTitleSize(0.15);
  hRatio_Up->GetXaxis()->SetLabelSize(0.10); 
  hRatio_Up->GetYaxis()->SetLabelSize(0.10); 
