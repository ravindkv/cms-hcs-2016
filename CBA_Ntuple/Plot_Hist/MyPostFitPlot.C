...
void example_stack(TString chName, TString chLable, TString charmCat){
  ...
  ch->SetTextSize(0.05);
  if(chName.Contains("ch1") || chName.Contains("ch2") ||chName.Contains("ch3") ) ch->AddText("#splitline{#mu + jets}{"+charmCat+"}");
  if(chName.Contains("ch4") || chName.Contains("ch5") ||chName.Contains("ch6") ) ch->AddText("#splitline{e + jets}{"+charmCat+"}");
  gPad->RedrawAxis();
  c1->Update();
