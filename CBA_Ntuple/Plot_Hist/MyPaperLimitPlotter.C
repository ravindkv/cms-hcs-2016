...
void CMS_lumi( TPad* pad, int iPeriod, int iPosX ){            
  ...
  if( !outOfFrame )
      ...
      else
	{
	  ...
	  latex.DrawLatex(posX_, 0.955, cmsText);
...
void LimitPlotter(TString CHANNEL="mu", TString CAT= "Cat1_Inc",
  ...
  if(draw8TeV) mg->SetMinimum(0.10);
  ...
  //if(CHANNEL=="mu_ele") chName = "\\ell + \\text{ jets}";
  ...
  leg->Draw();
  //if(CHANNEL=="mu_ele") l.DrawMathText(137, 2, chName);
  //else leg->AddEntry((TObject*)0, chName,"");
  leg->AddEntry((TObject*)0, chName,"");

   //pave text channel box
  TPaveText *ch = new TPaveText(0.25,0.80,0.45,0.88,"brNDC");
  ch->SetFillColor(19);
  ch->SetFillStyle(0);
  ch->SetLineColor(0);
  ch->SetTextSize(0.04);
  ...
  //gPad->SaveAs(outDir+"/"+outFile+".eps");
  c1->SaveAs(outDir+"/"+outFile+".pdf");
}
