...
class MyStackHisto{
  public :
    ...
    double errBandUp(int iBin, TH1F *hCentral, TH1F *hJESPlus, TH1F *hJERPlus, TH1F *bcTagPlus1, TH1F *bcTagPlus2, TH1F *bcTagPlus3, TH1F *PileupPlus, TH1F* TopPtPlus, TH1F* hQCD_dd, bool isDataDrivenQCD);
    double errBandDown(int iBin, TH1F *hCentral, TH1F *hJESMinus, TH1F *hJERMinus, TH1F *bcTagMinus1, TH1F *bcTagMinus2, TH1F *bcTagMinus3, TH1F* PileupMinus, TH1F* TopPtMinus, TH1F* hQCD_dd, bool isDataDrivenQCD);
    //unc graph
    TGraphAsymmErrors *UNCGRAPH(TH1F *hCentral, TH1F *hJESPlus, TH1F *hJESMinus, TH1F *hJERPlus, TH1F *hJERMinus, TH1F *bcTagPlus1, TH1F *bcTagMinus1, TH1F *bcTagPlus2, TH1F *bcTagMinus2,TH1F *bcTagPlus3, TH1F *bcTagMinus3, TH1F *PileupPlus, TH1F* PileupMinus, TH1F* TopPtPlus, TH1F* TopPtMinus, TH1F* hQCD_dd, bool isFullGraph = false, bool isRatioGraph = false, bool isDataDrivenQCD=false);
    //CMS guidelines
...
double MyStackHisto:: errBandUp(int iBin, TH1F *hCentral, TH1F *hJESPlus, TH1F *hJERPlus, TH1F *bcTagPlus1, TH1F *bcTagPlus2, TH1F *bcTagPlus3, TH1F *PileupPlus, TH1F* TopPtPlus, TH1F* hQCD_dd, bool isDataDrivenQCD){
  double errUp = sqrt(
          pow(fabs(hJESPlus->GetBinContent(iBin+1) - hCentral->GetBinContent(iBin+1)),2) +
		  ...
		  pow(fabs(TopPtPlus->GetBinContent(iBin+1) - hCentral->GetBinContent(iBin+1)),2) +
		  pow(hCentral->GetBinError(iBin+1),2));
  if(isDataDrivenQCD) errUp = sqrt(errUp*errUp + hQCD_dd->GetBinError(iBin+1));
  return errUp;
}

double MyStackHisto:: errBandDown(int iBin, TH1F *hCentral, TH1F *hJESMinus, TH1F *hJERMinus, TH1F *bcTagMinus1, TH1F *bcTagMinus2, TH1F *bcTagMinus3, TH1F* PileupMinus, TH1F *TopPtMinus,  TH1F* hQCD_dd, bool isDataDrivenQCD){
  double errDown =sqrt(
          pow(fabs(hCentral->GetBinContent(iBin+1) - hJESMinus->GetBinContent(iBin+1)),2) +
		  ...
		  pow(fabs(hCentral->GetBinContent(iBin+1) - TopPtMinus->GetBinContent(iBin+1)),2) +
		  pow(hCentral->GetBinError(iBin+1),2));
  if(isDataDrivenQCD) errDown = sqrt(errDown*errDown + hQCD_dd->GetBinError(iBin+1));
  return errDown;
}
...
TGraphAsymmErrors * MyStackHisto::UNCGRAPH(TH1F *hCentral, TH1F *hJESPlus, TH1F *hJESMinus, TH1F *hJERPlus, TH1F *hJERMinus, TH1F *bcTagPlus1, TH1F *bcTagMinus1, TH1F *bcTagPlus2, TH1F *bcTagMinus2,TH1F *bcTagPlus3, TH1F *bcTagMinus3, TH1F *PileupPlus, TH1F* PileupMinus, TH1F *TopPtPlus, TH1F* TopPtMinus, TH1F* hQCD_dd, bool isFullGraph = false, bool isRatioGraph = false, bool isDataDrivenQCD=false){
  ...
  for(int i=0; i<n1; i++){
    if(isFullGraph){
      ...
      errorU[i] = errBandUp(i, hCentral, hJESPlus, hJERPlus, bcTagPlus1, bcTagPlus2, bcTagPlus3, PileupPlus, TopPtPlus, hQCD_dd, isDataDrivenQCD);
      errorD[i] = errBandDown(i, hCentral, hJESMinus, hJERMinus, bcTagMinus1, bcTagMinus2, bcTagMinus3, PileupMinus, TopPtMinus, hQCD_dd, isDataDrivenQCD);
    }
    if(isRatioGraph){
      Yval[i]   = 1;
      double errUp = errBandUp(i, hCentral, hJESPlus, hJERPlus, bcTagPlus1, bcTagPlus2, bcTagPlus3, PileupPlus, TopPtPlus, hQCD_dd, isDataDrivenQCD);
      double errDown = errBandDown(i, hCentral, hJESMinus, hJERMinus, bcTagMinus1, bcTagMinus2, bcTagMinus3, PileupMinus, TopPtMinus, hQCD_dd, isDataDrivenQCD);
      //cout<<"bin = "<<i<<endl;
      //cout<<Yval[i]<<"\t"<<errorU[i]<<"\t"<<hCentral->GetBinContent(i+1)<<endl;
      double newBinCon = hCentral->GetBinContent(i+1);
      double newStatError = hCentral->GetBinError(i+1);
      double binCen = hCentral->GetBinCenter(i+1);
      if(newBinCon > 0){
    ...
    //printf("%d\t %f\t %f\t %f\t %f\n", i+1, newBinCon, binCen, 100*errUp/newBinCon, 100*errDown/newBinCon);
    }
...
vector<double> MyStackHisto::getQcdSF(TFile* fData, TFile* fTT, TFile* fST, TFile* fWJ, TFile* fDY, TFile* fVV, TString histDir, TString histName, TString xTitle, double xmin, double xmax){
  ...
  cout<<"========"<<endl;
  cout<<"hMC_RegC = "<<hMC_RegC->Integral()<<endl;
  cout<<"hData_RegC = "<<hData_RegC->Integral()<<endl;
  cout<<"hMC_RegD = "<<hMC_RegD->Integral()<<endl;
  cout<<"hData_RegD = "<<hData_RegD->Integral()<<endl;
